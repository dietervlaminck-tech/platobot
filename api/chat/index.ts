...
// Import the same system prompt — kept in sync with stream.ts
// In a production setup you'd extract this to a shared module
const SYSTEM_PROMPT = `Je bent Platobot, een gespecialiseerde AI-coach ontwikkeld voor Nyenrode Business University.
...
Als het gesprek net begint en de gebruiker heeft nog geen specifieke vraag of doel gedeeld, probeer dan op een natuurlijke manier te achterhalen waar ze mee bezig zijn en wat hun leerdoelen zijn.
...
VERPLICHT: Als een gebruiker in leiderschapsmodus een directe kennisvraag stelt, antwoord dan ALTIJD met: "Wat denk je zelf? Ik ben geen antwoordmachine zoals ChatGPT"
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    res.json({
      response: response.choices[0]?.message?.content || "Het spijt me, ik kon geen antwoord genereren.",
    });
  } catch (error: any) {
